...
function validateAsycnCustomRules(async_rules){
    ...
    if(async_rules && async_rules.length > 0){
        ...
        for(var i=0; i< async_rules.length;i++){
            
            var rule = async_rules[i];
        ...
        return Promise.all(async_methods).spread(function(res){
            return;
        }).catch(function(e){
...
function throwValidationError(err){
    return new Promise(function(){
      throw new Error(err); // throw new AppError.invalidArgumentsError(error_messages);
    });
}
