...
function initialize(params) {
    ...
    function extend(attributes) {
        ...
        Model.prototype.save = function save() {
            var self = this;
            return _knex.raw(getInsertSQL(self, deleteAllPropertiesWhichAreUndefined(self.getDBObject(self.input))));
        };
        ...
        Model.prototype.saveInBatch = function saveInBatch() {
            ...
            for (var index = 0; index < self.input.length; index++) {
                batch_statement += getInsertSQL(self, deleteAllPropertiesWhichAreUndefined(self.getDBObject(self.input[index])));
            }
        ...
        Model.prototype.fetch = function fetch() {
            ...
            if (arguments.length > 0) {
                ...
                return _knex(self.tableName).where(deleteAllPropertiesWhichAreUndefined(self.getDBObject(self.input))).select(_.toArray(arguments)).then(function(obj) {
                    if (obj.length > 0) {
                ...
                return _knex(self.tableName).where(deleteAllPropertiesWhichAreUndefined(self.getDBObject(self.input))).select().then(function(obj) {
                    if (obj.length > 0) {
                        return self.getObjectFromDBObject(obj.shift());
                    }
                    else {
        ...
        Model.prototype.getInBatch = function getInBatch() {
            var self = this;
            var result = [];
            return _knex(self.tableName).whereIn(((self.idAttribute) ? self.idAttribute : "id"), deleteAllPropertiesWhichAreUndefined(self.getDBObject(self.input))).then(function(output) {
