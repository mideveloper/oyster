    ...
    function extend(attributes) {
        ...
        function save(input) {
            ...
            return new Promise(function (resolve, reject) {
                getClient(server, pools, function(err, client) {
                    ...
                    client.collection(self.tableName).save(self.getMongoDataObject(input), function(err, output) {
                        if (err) {
                            reject(err);
                            return;
                        }
        ...
        Model.prototype.saveInBatch = function saveInBatch() {
            ...
            for (var index = 0; index < self.input.length; index++) {
                promises.push(save(self.input[index]));
            }
        ...
        function getArrayQuery(data){
            ...
            for (var property in data) {
                ...
                if (data[property] instanceof Array){
                    for(var index = 0; index < data[property].length; index++){
                        data[property][index] = self.getMongoDataObject(data[property][index]);
                    }
                    
                    query_obj[property] = { $each : data[property]};
                }
                else{
                    query_obj[property] = self.getMongoDataObject(data[property]);
                }
            }
            ...
            return new Promise(function (resolve, reject) {
                getClient(server, pools, function (err, client) {
                    ...
                    client.collection(self.tableName).update(
                        ...
                        { $addToSet: getArrayQuery(self.input) }, function (err, output) {
                            if (err) {
                                reject(err);
                                return;
                            }
        ...
        Mode.prototype.appendArrayItems = function appednArrayItems(where_clause) {
            var self = this;
            ...
            return new Promise(function (resolve, reject) {
                getClient(server, pools, function (err, client) {
                    ...
                    client.collection(self.tableName).update(
                        self.getMongoDataObject(where_clause),
                        { $push: getArrayQuery(self.input) }, function (err, output) {
                            if (err) {
                                reject(err);
                                return;
                            }
