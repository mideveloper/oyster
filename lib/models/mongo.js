        ...
        Model.prototype.update = function update(where_clause, set_fields, config) {
            ...
            return new Promise(function(resolve, reject) {
                getClient(server, pools, function(err, client) {
                    ...
                    if (config) {
                        client.collection(self.tableName).update(self.getDBObject(where_clause),
                        self.getDBObject(set_fields), config, function(err, output) {
                            if (err) {
                                reject(err);
                                return;
                            }
                        ...
                        client.collection(self.tableName).update(self.getDBObject(where_clause),
                        self.getDBObject(set_fields), function(err, output) {
                            if (err) {
                                reject(err);
                                return;
                            }
